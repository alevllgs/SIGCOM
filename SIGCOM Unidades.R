...
graba <- "BBDD Produccion/PERC/PERC 2021/11 Noviembre/01, 02, 03, 04 , 05, 06 y 07/06_COSTOS_INDIRECTOS.xlsx"

rehabilitacion_perc <- read_excel("BBDD Produccion/REM/Serie A/2021/2021-11 REM serie A.xlsx", sheet = "A28", range = "A168:B191", col_names = FALSE)

cmenor_perc <- read_excel("BBDD Produccion/REM/Serie BS/2021/2021-11 REM serie BS.xlsx", sheet = "B17", range = "B344:K363", col_names = FALSE, na = "0")

farmacia_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/95_Farmacia.xlsx")

aseo_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/03 M2_Distribucion de Pabellon_Mantencion.xlsx", sheet = "M2") %>% mutate(`PERC ASOCIADO`=CC, Cantidad = M2) %>% filter(`PERC ASOCIADO`!="648-ASEO") %>% select(`PERC ASOCIADO`, Cantidad) %>% mutate(Item ="648_1-ASEO | Metro cuadrado")

aseo_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/03 M2_Distribucion de Pabellon_Mantencion.xlsx", sheet = "M2") %>% mutate(`PERC ASOCIADO`=CC, Cantidad = M2) %>% filter(`PERC ASOCIADO`!="648-ASEO") %>% select(`PERC ASOCIADO`, Cantidad) %>% mutate(Item ="648_1-ASEO | Metro cuadrado")

anatomia_patologica_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/96_Anatomia_Patologica.xlsx", sheet = "Noviembre 2021", range = "C4:D80") %>% mutate(Cantidad = `544_1-ANATOMÍA PATOLÓGICA | Estudio`) %>% filter(`PERC ASOCIADO`!="544-ANATOMÍA PATOLÓGICA" & Cantidad>0) %>% select(`PERC ASOCIADO`, Cantidad) %>% mutate(Item ="544_1-ANATOMÍA PATOLÓGICA | Estudio")

imagenologia_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/92_Imagenologia.xlsx", range = "C1:D79")  %>% mutate(Cantidad = `541_1-TOMOGRAFÍA | Estudio`) %>% filter(`PERC ASOCIADO`!="542-IMAGENOLOGÍA" & Cantidad>0) %>% select(`PERC ASOCIADO`, Cantidad) %>% mutate(Item ="542_1-IMAGENOLOGÍA | Estudio")

laboratorio_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/93_Laboratorio.xlsx", range = "C1:I79") %>% mutate(Cantidad = Totales) %>% filter(`PERC ASOCIADO`!="518-LABORATORIO CLÍNICO" & Cantidad>0) %>% select(`PERC ASOCIADO`, Cantidad) %>% mutate(Item ="518_1-LABORATORIO CLÍNICO | Exámen")

UMT_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/91_UMT.xlsx", 
                       range = "C1:E78", na = "0") %>% mutate_all(., ~replace(., is.na(.), 0))

esterilizacion_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/90_Esterilizacion.xlsx", 
                                  range = "C1:F77")

transporte_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/97_Transporte.xlsx", 
                       range = "C1:F82", na = "0") %>% mutate_all(., ~replace(., is.na(.), 0))

psicosocial_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/94_Psicosocial.xlsx", 
...
alimentacion_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/98_Alimentacion.xlsx", 
...
equipos_medicos_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Insumos de Informacion/99_Equipos_Medicos.xlsx", 
...
procedimientos_perc <- read_excel("BBDD Produccion/PERC/PERC 2021/11 Noviembre/Complemento Subir/05.xlsx") %>% 
